...
def check_status(
    ...
    *,
    ...
    result: dict[str, object] = {
        "schema": "rosetta.staging-status.v1",
    ...
    try:
        ...
        if health.get("schema") != "rosetta.observer-health.v1":
        ...
        if health.get("release") != expected_release.removeprefix("v"):
            reasons.append("release_mismatch")
        protocol_digest = health.get("protocol_digest")
        if not isinstance(protocol_digest, str) or DIGEST.fullmatch(protocol_digest) is None:
    ...
    if protocol_digest is not None:
        ...
        try:
            ...
            if evidence.get("release") != expected_release.removeprefix("v"):
        ...
        try:
            ...
            if row is None:
                ...
                if row[0] != expected_release.removeprefix("v"):
                    reasons.append("database_release_mismatch")
                if int(row[1]) < min_observations:
                    reasons.append("observation_count_too_low")
